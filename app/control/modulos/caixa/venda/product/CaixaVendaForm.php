 ...
 */
class CaixaVendaForm extends TWindow
{
    protected $form; // form
    ...
    function __construct()
    {
        ...
        $button = new TActionLink('', new TAction(['PessoaForm', 'onEdit']), 'green', null, null, 'fa:plus-circle');
        ...
        $product_detail_product_id = new TDBUniqueSearch('product_detail_product_id', 'DBUNIDADE', 'Produto', 'id', 'nome');
    ...
    public static function onProductChange( $params )
    {
        if( !empty($params['product_detail_product_id']) )
        {
            try
            {
                ...
                TForm::sendData('form_Sale', (object) ['product_detail_price' => $product->valor ]);
                TTransaction::close();
            }
            catch (Exception $e)
            {
