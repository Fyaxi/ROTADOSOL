 ...
 * 
 */
class FormaRecebimentoForm extends TPage
{
    protected $form; // form
    ...
    function __construct()
    {
        ...
        $this->setAfterSaveAction( new TAction(['FormaRecebimentoList', 'onReload'], ['register_state' => 'true']) );
        
        $this->setDatabase('DBUNIDADE');              // defines the database
        $this->setActiveRecord('FormaRecebimento');     // defines the active record
        
        // creates the form
        $this->form = new BootstrapFormBuilder('form_FormaRecebimento');
    ...
    function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $object = new FormaRecebimento($key);

                if($object->id == 1)
                {
                    TEntry::disableField('form_FormaRecebimento', 'nome');
                    TRadioGroup::disableField('form_FormaRecebimento', 'ativo');
                }
