 ...
 * 
 */
class ReceberEdit extends TWindow
{
    protected $form; // form
    protected $fieldlist;
    ...
    function __construct($param)
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('formEdit_ContaReceberPg');
        ...
        $valor_total->onBlur   = 'calculate_bmi()';
        $desconto->onBlur = 'calculate_bmi()';
        
        TScript::create('calculate_bmi = function() {
            if (parseFloat(document.formedit_ContaReceberPg.valor_total.value) > 0 || parseFloat(document.formedit_ContaReceberPg.desconto.value) > 0)
            {
                formedit_ContaReceberPg.valor.value = parseFloat(formedit_ContaReceberPg.valor_total.value) - parseFloat(formedit_ContaReceberPg.desconto.value);
            }
            
            if (parseFloat(document.formedit_ContaReceberPg.valor_total.value) > 0 || parseFloat(document.formedit_ContaReceberPg.desconto.value) = 0)
            {
                formedit_ContaReceberPg.valor.value = parseFloat(formedit_ContaReceberPg.valor_total.value);
            }
        };');
        ...
        $recebimentos_id = new TDBUniqueSearch('list_servico_id[]', 'DBUNIDADE', 'FormaRecebimento', 'id', 'nome', null, $criteria);
        ...
        $this->form->addContent( [ TElement::tag('h5', 'Entradas Financeiras', [ 'style'=>'background: whitesmoke; padding: 5px; border-radius: 5px; margin-top: 5px'] ) ] );
    ...
    function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $items  = ContaReceberPg::where('recebimento_id', '=', $key)->load();
                ...
                if(!empty($param['list_servico_id'][0]))
                {
                    ...
                    if($valor_fatura == $total_lancamentos_r)
                    {
                        if( $valor_fatura >= 1 AND $total_lancamentos_r >= 1)
                        {
                            ...
                            $data = new stdClass;
                            $data->id = $master->id;
                            TForm::sendData('formedit_ContaReceberPg', $data);

                            ContaReceberPg::where('recebimento_id', '=', $master->id)->delete();
                            
                            foreach( $param['list_servico_id'] as $row => $formarecebimento)
                            {
                                $detail = new ContaReceberPg;
                    ...
                    {
                        throw new Exception('<span style="color:black;font-weight: bold;">VALOR TOTAL DAS ENTRADAS</span><BR>NÃO CONCILIA COM O VALOR TOTAL DO RECEBIMENTO');
                    }
                }
                else
                {
