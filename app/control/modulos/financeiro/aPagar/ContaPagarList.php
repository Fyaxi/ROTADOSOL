...
class ContaPagarList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->setOrderCommand('pessoa->nome_fantasia', '(SELECT nome_fantasia FROM pessoa WHERE pessoa.id=conta_pagar.pessoa_id)');
        ...
        $column_pessoa_id = new TDataGridColumn('pessoa->nome_fantasia', 'Favorecido', 'left');
        ...
        $action1 = new TDataGridAction(['PagarView', 'onView'], ['id'=>'{id}', 'register_state' => 'false']);
        $action2 = new TDataGridAction(['PagarEdit', 'onEdit'], ['id'=>'{id}', 'register_state' => 'false']);
        $action3 = new TDataGridAction([$this, 'onPagar'], ['id'=>'{id}']);
        ...
        $panel = new TPanelGroup('<small>pagamentos</small>', 'white');
     ...
     */
    public static function onPagar( $param )
    {
        TTransaction::open('DBUNIDADE');

        $form = new BootstrapFormBuilder('input_form_2');

        $items  = ContaPagarPg::where('recebimento_id', '=', $param['id'])->load();

        if ($items)
        {
            $ContaPagar = new ContaPagar($param['id']);
            if(empty($ContaPagar->dt_pagamento))
            {
                $id = new TEntry('id');
                $dt_pagamento = new TDate('dt_pagamento');
                ...
                $valor->setValue($ContaPagar->valor);
                $valor_total->setValue($ContaPagar->valor_total);
                $desconto->setValue($ContaPagar->desconto);
                ...
                $form->addAction('Finalizar Pagamento', new TAction([__CLASS__, 'onLiquidarPagamento']), 'fa:arrow-circle-down green');
                $form->addAction('Cancelar', new TAction([__CLASS__, 'onCancelarOperacao']), 'fa:times red');

                // show the input dialog
                new TInputDialog('Realizar Pagamento', $form);
        ...
        {
            TToast::show('warning', 'Não há recebimentos vinculados para o pagamento.', 'top center', 'far:check-circle' );
        }
     ...
     */
    public static function onLiquidarPagamento( $param )
    {
        TTransaction::open('DBUNIDADE');
        ...
        if(!empty($param['ValorRecebimento']))
        {
            ...
            $ValorConta = $ContaPagar->valor;
            $ValorContaTotal = $ContaPagar->valor_total;

            if(!empty($param['desconto']))
            {
                if($ValorRecebimentoDesconto == $ContaPagar->desconto)
                {
                    $NovoValorConta = $ValorContaTotal - $ValorRecebimentoDesconto;
                    if($ValorRecebimento == $NovoValorConta)
                    {
                        $ContaPagar->dt_pagamento = date("Y-m-d H:i:s");
                        $ContaPagar->store();
                ...
                else
                {
                    ...
                    {
                        $ContaPagar->dt_pagamento = date("Y-m-d H:i:s");
                        $ContaPagar->desconto = $ValorRecebimentoDesconto;
                        $ContaPagar->valor = $ValorRecebimento;
                        //$ContaPagar->obs = 'Desconto Alterado De: ('.$ContaPagar->desconto.') > ('.$ValorRecebimentoDesconto.')';
                        $ContaPagar->store();
                        $pos_action = new TAction(['ContaPagarList', 'onReload'], ['register_state' => 'true']);
                        new TMessage('info', 'Liquidação <b>'.$param['id'].'</b> Ajustada!', $pos_action);
                    }
    ...
    public static function onEstornarPg( $param )
    {
        ...
        $Lancamentos  = ContaPagar::where('id', '=', $param['id'])->load();

        if ($Lancamentos)
        {
            $ContaPagar = new ContaPagar($param['id']);

            if($ContaPagar->ativo == 'Y' AND $ContaPagar->dt_pagamento !== NULL)
            {
                ContaPagarPg::where('recebimento_id', '=', $param['id'])->delete();
                $ContaPagar->dt_pagamento = NULL;
                $ContaPagar->store();
                $pos_action = new TAction(['ContaPagarList', 'onReload'], ['register_state' => 'true']);
                new TMessage('info', 'Pagamento Estornado!', $pos_action);
            }
            else
            {
    ...
    public function onCancel($param)
    {
        try
        {
            ...
            $PagamentosEventos = ContratoPagamento::where('contrato_id', '=', $contapagar->contrato_id)
                                                  ->where('pessoa_id', '=', $contapagar->pessoa_id)
                                                  ->load();

            foreach ($PagamentosEventos as $Pagamentos)
            {
                $AlteraPagamentos = new ContratoPagamento($Pagamentos->id);
                $AlteraPagamentos->status = 'N';
                $AlteraPagamentos->store();
            }
