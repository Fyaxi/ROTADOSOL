...
class ColaboradorForm extends TWindow
{
    ...
    public function __construct( $param )
    {
        ...
        $salario_inicio = new TNumeric('salario_inicio', 2, ',', '.');
        $salario_fim = new TNumeric('salario_fim', 2, ',', '.');
        ...
        $GrupoPagamento_id = new TDBRadioGroup('GrupoPagamento_id', 'DBUNIDADE', 'GrupoPagamento', 'id', 'nome');
        ...
        $GrupoPagamento_id->setLayout('vertical TDBCheckGroup');
        ...
        $row->layout = ['col-sm-2', 'col-sm-5', 'col-sm-3', 'col-sm-2' ];
        ...
        $row = $this->form->addFields( [ new TLabel('<br>RG'),  $codigo_municipal ],
                                       ...
                                       [ new TLabel('<br>Email <span style="color:red">*</span>'),   $email ]);
        ...
        $row = $subform->addFields( [ new TLabel('<br>Chave Pix'),  $codigo_estadual ], );
        $row->layout = ['col-sm-12'];

        $row = $subform->addFields( [ new TLabel('<br>Margem Salarial de'),  $salario_inicio ],
                                    [ new TLabel('<br>Até'),  $salario_fim ] );
        $row->layout = ['col-sm-6', 'col-sm-6'];
        ...
        $row = $subform->addFields( [ new TLabel('<br>Grupo <span style="color:red">*</span>'),  $grupo_id ] );
        $row->layout = ['col-sm-12'];

        $row = $subform->addFields( [ new TLabel('<br>Grupo Pagamento <span style="color:red">*</span>'),  $GrupoPagamento_id ] );
        $row->layout = ['col-sm-12'];
        ...
        //$this->form->addFields( [ new TLabel('Nome') ], [ $nome_fantasia ], [ new TLabel('Apelido') ], [ $nome ] );

        //$this->form->addFields( [ new TLabel('Papéis')], [ $papeis_id ], [ new TLabel('Grupo') ], [ $grupo_id ] );
        //$this->form->addFields( [ new TLabel('Chave PIX') ], [ $codigo_estadual ], [ new TLabel('RG') ], [ $codigo_municipal ] );
        ...
        $codigo_estadual->setSize('100%');  // Chave Pix
        ...
        $salario_inicio->setSize('100%');
        $salario_fim->setSize('100%');
    ...
    public function onSave( $param )
    {
        try
        {
            ...
            $object->salario_inicio = TDateTime::convertToMask($param['salario_inicio'], 'dd/mm/yyyy', 'yyyy-mm-dd');
            $object->salario_fim = TDateTime::convertToMask($param['salario_fim'], 'dd/mm/yyyy', 'yyyy-mm-dd');
    ...
    public static function onChangeEstado($param)
    {
        try
        {
            ...
            if (!empty($param['estado_id']))
            {
                ...
                TDBCombo::reloadFromModel('form_Colaborador', 'cidade_id', 'DBUNIDADE', 'Cidade', 'id', '{nome} ({id})', 'nome', $criteria, TRUE);
            }
            else
            {
