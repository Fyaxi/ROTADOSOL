...
class ContratoDashboard extends TPage
{
    ...
    function __construct()
    {
        ...
        try
        {
            ...
            $total_inativos  = Contrato::where('ativo','=','N')->count();
            ...
            $top_clientes    = ViewContratos::where('ativo','=','Y')->groupBy('nome_cliente')->orderBy('total', 'desc')->sumBy('total');
            $old_clientes    = ViewContratos::where('ativo','=','Y')->take(5)->orderBy('dt_inicio', 'desc')->load();
            TTransaction::close();
            
            
            $indicator1 = new THtmlRenderer('app/resources/info-box.html');
            $indicator1->enableSection('main', ['title' => 'Festas a Realizar', 'icon' => 'check-double', 'background' => 'green', 'value' => $total_ativos ] );
            ...
            $div->add( TElement::tag('div', TPanelGroup::pack('Festas Por Valor', $table1),     ['class' => 'col-sm-6']) );
            $div->add( TElement::tag('div', TPanelGroup::pack('Listagem De Festas', $table2),     ['class' => 'col-sm-6']) );
