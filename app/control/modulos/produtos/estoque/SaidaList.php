...
class SaidaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->setActiveRecord('Saida');   // defines the active record
        ...
        $this->form->setFormTitle('Saida De Estoque');
        ...
        $this->form->addActionLink('Inserir Nova Saida', new TAction(['SaidaForm', 'onEdit']), 'fa:plus green');
        ...
        $column_dt_fatura = new TDataGridColumn('dt_fatura', 'Dt Saida', 'center');
    ...
    public function onCancel($param)
    {
        try
        {
            ...
            $venda = new Saida($param['id']);
            
            if ($venda->ativo == 'Y')
            {
                $fcr = SaidaContaReceber::where('fatura_id', '=', $venda->id)->first();
                ...
                $pesq_venda = SaidaItem::where('fatura_id', '=', $venda->id)->load();
                ...
                new TMessage('info', 'Saida cancelada');
            }
