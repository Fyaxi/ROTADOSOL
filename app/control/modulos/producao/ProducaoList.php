...
class ProducaoList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->setActiveRecord('Producao');   // defines the active record
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_Producao');
        $this->form->setFormTitle('Producao');
        ...
        $this->form->addActionLink('Inserir Nova Produção', new TAction(['ProducaoForm', 'onEdit']), 'fa:plus green');
        ...
        $column_dt_fatura = new TDataGridColumn('dt_fatura', 'Dt Producao', 'center');
        ...
        $column_financeiro_gerado->setTransformer( function($value, $object) {
            ...
            $fcc = ProducaoContaPagar::where('fatura_id', '=', $object->id)->first();

            if ($object->financeiro_gerado == 'Y')
            {
                if ($fcc)
                {
                    $conta_receber = ContaPagar::find($fcc->conta_receber_id);
                    if ($conta_receber)
                    {
                        if (!empty($conta_receber->dt_pagamento))
                        {
                            ...
                            {
                                $value = 'Pagamento <b>nº ('.$conta_receber->id.')</b> a maior';
                                $label = 'success';
                            }elseif( $object->total > $conta_receber->valor)
                            {
                                $value = 'Pagamento <b>nº ('.$conta_receber->id.')</b> a menor';
                                $label = 'success';
                            }
                            else{
                                $value = 'Pago';
                            ...
                            $value = 'Pagamento <b>nº ('.$conta_receber->id.')</b> Divergente';
                            $label = 'warning';
                        }
        ...
        $action1 = new TDataGridAction(['ProducaoForm', 'onEdit'], ['id'=>'{id}']);
    ...
    public function onCancel($param)
    {
        ...
        {
            TTransaction::open(TSession::getValue('unit_database'));
            $producao = new Producao($param['id']);
            
            if ($producao->ativo == 'Y')
            {
                $fcr = ProducaoContaPagar::where('fatura_id', '=', $producao->id)->first();
                
                if ($fcr)
                {
                    $contareceber = ContaPagar::find($fcr->conta_receber_id);
                ...
                $pesq_producao = ProducaoItem::where('fatura_id', '=', $producao->id)->load();
                foreach ($pesq_producao as $producao_item)
                {
                    //print 'Produto id:'.$producao_item->servico_id.'<br>';
                    //print 'Quantidade:'.$producao_item->quantidade.'<br>';

                    $qnt_produto = explode(".", $producao_item->quantidade);
                    ...
                    $produto = new Produto($producao_item->servico_id);
                ...
                new TMessage('info', 'Produção Cancelada!');
            }
